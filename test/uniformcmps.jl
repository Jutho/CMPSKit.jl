...
@testset "UniformCMPS: environments and gauging with bond dimension $D" for D in Dlist
    for T in (Float64, ComplexF64)
        ...
        ρL, λL, infoL = leftenv(Ψ; krylovdim = max(D^2,50), tol = 1e-12)
        @test norm(Q'*ρL + ρL*Q + R1'*ρL*R1 + R2'*ρL*R2 - 2*λL*ρL) <= 10*infoL.normres[1]
        @test ρL == ρL'

        ρR, λR, infoR = rightenv(Ψ; krylovdim = max(D^2,50), tol = 1e-12)
        ...
        Ψn = copy(Ψ)
        ρL2, = leftenv!(Ψn; krylovdim = max(D^2,50), tol = 1e-12)
        @test ρL2 ≈ ρL
        Qn = Ψn.Q
        R1n, R2n = Ψn.Rs
        ...
        Ψn = copy(Ψ)
        ρR2, = rightenv!(Ψn; krylovdim = max(D^2,50), tol = 1e-12)
        @test ρR2 ≈ ρR
        Qn = Ψn.Q
        R1n, R2n = Ψn.Rs
        ...
        Ψn = copy(Ψ)
        ρL2, ρR2 = environments!(Ψn; krylovdim = max(D^2,50), tol = 1e-12)
        @test ρL2 ≈ ρL/sqrt(Z)
        @test ρR2 ≈ ρR/sqrt(Z)
        Qn = Ψn.Q
        R1n, R2n = Ψn.Rs
